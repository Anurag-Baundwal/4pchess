...
class Board; // FIX 1: Moved Board forward declaration up
class Move;  // FIX 1: Added Move forward declaration

int SeeRecursive(const Board& board, const int piece_evaluations[6], int sq, Team attacker_team, Bitboard occupied);
int StaticExchangeEvaluationCapture(const int piece_evaluations[6], Board& board, const Move& move); // FIX 1: Added forward declaration
...
class Move {
 public:
  ...
  int ApproxSEE(const Board& board, const int* piece_evaluations) const; // FIX 4: Added const
...
class Board {
 public:
  ...
  // FIX 2: Added friend declarations to grant access to private members
  friend int chess::SeeRecursive(const Board&, const int[6], int, Team, Bitboard);
  friend int chess::StaticExchangeEvaluationCapture(const int[6], Board&, const Move&);
...
// FIX 3: Removed redundant declaration from here. It is now forward-declared at the top.
