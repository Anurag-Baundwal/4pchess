...
class BoardLocation {
 public:
  ...
  uint8_t GetIndex() const { return loc_; } // OPTIMIZATION: Raw index access for 1D arrays
...
struct hash<chess::BoardLocation> {
  std::size_t operator()(const chess::BoardLocation& x) const {
    // Reverted to fast index hashing for O(1) Zobrist updates and array lookups
    return std::hash<uint8_t>()(x.GetIndex());
  }
};
} // namespace std
