# train5.py
# eliminate contamination of validation set
import argparse
import gc
import glob
...
def data_generator(train_data_dirs, immediate=False, search_depth_for_signature=None):
  def gen():
    data_dirs_list = train_data_dirs
    ...
    for data_dir in data_dirs_list:
      # Expecting specific filenames in these pre-split training directories
      # In 'train-new' mode, these are 'train_data_gen_X' dirs with board_0.csv etc.
      # In 'train-existing' mode, these are user-provided dirs, glob might be needed if structure varies
      
      # Try specific filenames first (for 'train-new' mode structure)
      b_fp = os.path.join(data_dir, 'board_0.csv')
      s_fp = os.path.join(data_dir, 'score_0.csv')
      p_fp = os.path.join(data_dir, 'per_depth_score_0.csv')

      if os.path.exists(b_fp) and os.path.exists(s_fp) and os.path.exists(p_fp):
          board_filepaths.append(b_fp)
          score_filepaths.append(s_fp)
          per_depth_score_filepaths.append(p_fp)
      else:
          # Fallback to glob for 'train-existing' mode or other structures
          # This part is similar to the original glob logic
          ...
          if current_dir_board_fps and len(current_dir_board_fps) == len(current_dir_score_fps) == len(current_dir_pds_fps):
              board_filepaths.extend(current_dir_board_fps)
              score_filepaths.extend(current_dir_score_fps)
              per_depth_score_filepaths.extend(current_dir_pds_fps)
          else:
              print(f"Warning: Data files missing or mismatched in {data_dir} (tried specific and glob). Skipping.")
              continue
    
    if not board_filepaths:
        print("Error: No data files found by data_generator.")
    ...
    for board_filepath, score_filepath, per_depth_score_filepath in zip(
      ...
      try:
        ...
        if not (board_np.shape[0] == score_np.shape[0] == per_depth_score_np.shape[0]):
            print(f"Warning: Mismatch in number of samples in file {board_filepath}")
            continue
  ...
  if not results_list:
      raise ValueError("No data could be loaded by the data generator (immediate=True).")
  def immediate_gen():
